...
public class BusManager {
	...
	public static ArrayList<BerichtLogging> getLoggingInfo(int vanaf, int tot) {
		int aantalBerichten = ETAPersistence.getAantalBerichten();
		tot = (tot >= aantalBerichten) ? aantalBerichten : tot;
		ArrayList<BerichtLogging> berichtLogging = new ArrayList<BerichtLogging>();
		if (vanaf < aantalBerichten-1) {
			for (int i = vanaf; i < tot; i++) {
				BerichtLogging nextBL = new BerichtLogging();
				Bericht nextBericht = ETAPersistence.getBericht(i);
				nextBL.bedrijf = nextBericht.bedrijf;
				nextBL.busID = nextBericht.busID;
				nextBL.eindpunt = nextBericht.eindpunt;
				nextBL.lijnNaam = nextBericht.lijnNaam;
				nextBL.tijd = nextBericht.tijd;
				nextBL.aantalETAs = nextBericht.ETAs.size();
				berichtLogging.add(nextBL);
			}
		}
		return berichtLogging;
	}
